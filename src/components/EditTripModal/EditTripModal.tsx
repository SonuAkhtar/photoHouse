...
export default function EditTripModal({ trip, onClose, onUpdated }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      const { data } = await updateTrip(trip.id, { place, region, dates, summary, accent, tags });
    ...
    <div className="edit-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      >
        <div className="edit-modal_head">
          ...
          <button className="edit-modal_close" onClick={onClose} aria-label="Close">✕</button>
        </div>
        ...
        <form className="edit-modal_form" onSubmit={handleSubmit} noValidate>
          <div className="edit-modal_row">
            <div className="edit-modal_field">
              <label className="edit-modal_label">Place <span className="edit-modal_req">*</span></label>
              <input className="edit-modal_input" value={place} onChange={e => setPlace(e.target.value)} placeholder="e.g. Santorini" />
            </div>
            <div className="edit-modal_field">
              <label className="edit-modal_label">Region <span className="edit-modal_req">*</span></label>
              <input className="edit-modal_input" value={region} onChange={e => setRegion(e.target.value)} placeholder="e.g. Cyclades · Greece" />
            </div>
          </div>

          <div className="edit-modal_field">
            <label className="edit-modal_label">Dates <span className="edit-modal_req">*</span></label>
            <input className="edit-modal_input" value={dates} onChange={e => setDates(e.target.value)} placeholder="e.g. June 14 – 21, 2024" />
          </div>

          <div className="edit-modal_field">
            <label className="edit-modal_label">Description <span className="edit-modal_req">*</span></label>
            <textarea className="edit-modal_textarea" rows={4} value={summary} onChange={e => setSummary(e.target.value)} placeholder="Describe this trip…" />
          </div>

          <div className="edit-modal_field">
            <label className="edit-modal_label">Tags <span className="edit-modal_hint">(max 5)</span></label>
            ...
            <div className="edit-modal_tag-row">
              ...
              />
              <button type="button" className="edit-modal_tag-add" onClick={addTag} disabled={tags.length >= 5}>
                Add
              </button>
            </div>
          </div>
          ...
          <div className="edit-modal_actions">
            <button type="button" className="edit-modal_cancel" onClick={onClose}>Cancel</button>

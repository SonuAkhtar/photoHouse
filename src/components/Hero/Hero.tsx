...
export default function Hero({ count }: HeroProps) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      section.style.setProperty('--cx', `${((e.clientX - rect.left) / rect.width) * 100}%`);
      section.style.setProperty('--cy', `${((e.clientY - rect.top) / rect.height) * 100}%`);
    };
  ...
  useEffect(() => {
    ...
    const tween = gsap.to({ n: 0 }, {
      ...
      onUpdate() {
        if (countRef.current) {
          countRef.current.textContent = String(Math.round((this as any).targets()[0].n)).padStart(2, '0');
        }
      },
    });
    return () => { tween.kill(); };
    ...
    <section className="hero" ref={sectionRef}>
      ...
      <div className="hero__body">
        ...
        <div className="hero__text-col">
            ...
            {['PHOTO', 'HOUSE'].map((word, wi) => (
              <div key={wi} className="hero__line">
                {word.split('').map((ch, li) => (
                  <span key={`${wi}-${li}`} className="hero__ch">{ch}</span>
                ))}
              </div>
            ))}
          </div>
          ...
          <div className="hero__count">
            <span className="hero__count-num" ref={countRef}>00</span>
            <span className="hero__count-label">Journeys<br />Documented</span>
          </div>
        </div>
        ...
        <div className="hero__collage">
          <div ref={photo1Ref} className="hero__photo hero__photo--1">
            <img src={covers[0].cover} alt={covers[0].place} draggable={false} />
          ...
          <div ref={photo2Ref} className="hero__photo hero__photo--2">
            <img src={covers[1].cover} alt={covers[1].place} draggable={false} />
            <span className="hero__photo-label">{covers[1].place}</span>
            <div className="hero__photo-sheen" aria-hidden="true" />
          </div>

          <div ref={photo3Ref} className="hero__photo hero__photo--3">
            <img src={covers[2].cover} alt={covers[2].place} draggable={false} />
      ...
      <div className="hero__marquee-wrap" aria-hidden="true">
        <div className="hero__marquee-track">
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((place, i) => (

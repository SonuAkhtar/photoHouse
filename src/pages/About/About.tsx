...
import { fetchProfile, fetchTrips, type ApiProfile, type ApiTrip } from '../../services/api';
import './About.css';
...
function computeStats(trips: ApiTrip[]) {
  ...
  const yearsShooting  = years.length > 0 ? Math.max(1, currentYear - Math.min(...years)) : 0;
...
export default function About() {
  ...
  useEffect(() => {
    Promise.all([fetchProfile(), fetchTrips()]).then(([{ data: prof }, { data: trps }]) => {
      setProfile(prof);
      setTrips(trps);
    }).catch(console.error);
  }, []);
  ...
  const statItems = [
    {
      value: stats.destinations > 0 ? String(stats.destinations).padStart(2, '0') : '—',
      label: 'Destinations',
    },
    {
    ...
    <main className="about">
      <div className="about_inner">
        ...
        <motion.h1 className="about_heading" {...fadeUp(0.2)}>
          Chasing light<br />across the world
        </motion.h1>
          ...
          <motion.div className="about_col" {...fadeUp(0.4)}>
            {hasBio ? (
              bioParagraphs.map((para, i) => (
                <p key={i} className="about_bio">{para}</p>
              ))
            ) : (
              <p className="about_bio about_bio-empty">
                Add a bio on your{' '}
                <Link to="/profile" className="about_profile-link">profile page</Link>
          ...
          <motion.div className="about_gear" {...fadeUp(0.76)}>
            ...
            <div className="about_gear-list">
              {profile!.gear.map(item => (
                <span key={item} className="about_gear-item">{item}</span>
              ))}
            </div>
          </motion.div>
        )}
        ...
        {!hasQuote && !hasGear && (
          <motion.p className="about_setup-hint" {...fadeUp(0.64)}>
            Complete your{' '}
            <Link to="/profile" className="about_profile-link">profile</Link>

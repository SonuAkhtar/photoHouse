...
import {
  fetchProfile, saveProfile, fetchTrips,
  type ApiProfile, type ApiInterest, type ApiTrip,
} from '../../services/api';
import './Profile.css';
...
function computeStats(trips: ApiTrip[]) {
  ...
  const yearsShooting = years.length > 0
    ? Math.max(1, currentYear - Math.min(...years))
    : 0;
...
export default function Profile() {
  ...
  const handleGearKey = (e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') { e.preventDefault(); addGear(); }
  };

  const removeGear = (item: string) => setGear(prev => prev.filter(g => g !== item));

  // ── Interests ─────────────────────────────────────────────────────────────

  const addInterest = () =>
    setInterests(prev => [...prev, { id: crypto.randomUUID(), title: '', body: '' }]);

  const updateInterest = (id: string, field: 'title' | 'body', value: string) =>
    setInterests(prev => prev.map(it => it.id === id ? { ...it, [field]: value } : it));
        ...
        <motion.div className="profile_stats-row" {...fadeUp(0.2)}>
          ...
          <div className="profile_stats">
            <div className="profile_stat">
              <span className="profile_stat-value">{String(stats.destinations).padStart(2, '0')}</span>
              <span className="profile_stat-label">Destinations</span>
            </div>
            <div className="profile_stat">
              <span className="profile_stat-value">{stats.yearsShooting > 0 ? `${stats.yearsShooting}+` : '—'}</span>
              <span className="profile_stat-label">Years shooting</span>
            </div>
            <div className="profile_stat">
              <span className="profile_stat-value">{stats.totalPhotos > 0 ? `${stats.totalPhotos}+` : '—'}</span>
        ...
        <form className="profile_form" onSubmit={handleSave} noValidate>
          ...
          <motion.div className="profile_section" {...fadeUp(0.28)}>
            ...
            <textarea
              ...
              rows={5}
            />
            <p className="profile_hint">Shown on the About page. Use a blank line to separate paragraphs.</p>
          </motion.div>
          ...
          <motion.div className="profile_section" {...fadeUp(0.4)}>
            ...
            <div className="profile_gear-input-row">
              ...
              />
              <button type="button" className="profile_gear-add" onClick={addGear}>
                Add
              </button>
            </div>
            <p className="profile_hint">Press Enter or click Add. Shown as tags on the About page.</p>
          </motion.div>
          ...
          <motion.div className="profile_section" {...fadeUp(0.46)}>
            <div className="profile_section-head">
              ...
              <button type="button" className="profile_add-interest" onClick={addInterest}>
                + Add interest
              </button>
            </div>
            <p className="profile_hint" style={{ marginBottom: '1rem' }}>
              Each card appears on the Interests page in the same 3-column grid format.
            </p>

            {interests.length === 0 && (
              <p className="profile_empty-hint">No interests yet — click "+ Add interest" to create your first card.</p>
            )}
              ...
              {interests.map((it, i) => (
                <div key={it.id} className="profile_interest-card">
                  <div className="profile_interest-card-head">
                    <span className="profile_interest-index">{String(i + 1).padStart(2, '0')}</span>
                    <button
                      type="button"
                  ...
                  <input
                    ...
                    type="text"
                    value={it.title}
                    onChange={e => updateInterest(it.id, 'title', e.target.value)}
                    placeholder="Interest title"
                  />
                  <textarea
                    className="profile_textarea profile_interest-body"
                    value={it.body}
                    onChange={e => updateInterest(it.id, 'body', e.target.value)}
          ...
          <motion.div className="profile_save-bar" {...fadeUp(0.5)}>
            {saved && <span className="profile_saved-msg">Changes saved ✓</span>}
            <button type="submit" className="profile_save-btn" disabled={saving}>

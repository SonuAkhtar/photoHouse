...
function getStats(trips: ApiTrip[]) {
  ...
  years.forEach(y => { perYear[y] = (perYear[y] || 0) + 1; });
  ...
  trips.forEach(t => (t.tags ?? []).forEach(tag => {
    tagCounts[tag] = (tagCounts[tag] || 0) + 1;
  }));

  const yearList = Object.entries(perYear).sort(([a], [b]) => Number(a) - Number(b));
  const maxPerYear = Math.max(...yearList.map(([, n]) => n), 1);

  const regionList = Object.entries(regions).sort(([, a], [, b]) => b - a).slice(0, 6);
  const tagList    = Object.entries(tagCounts).sort(([, a], [, b]) => b - a).slice(0, 8);
  ...
  return { totalPhotos, yearList, maxPerYear, regionList, tagList, yearsShooting };
}
  ...
  return (
    <main className="stats-page">
      <div className="stats-page_inner">
        <motion.p className="stats-page_eyebrow" {...fadeUp(0.05)}>By the numbers</motion.p>
        <motion.h1 className="stats-page_heading" {...fadeUp(0.15)}>Your Journey Stats</motion.h1>
        <motion.div className="stats-page_divider" {...fadeUp(0.25)} />

        {loading ? (
          <div className="stats-loading"><span className="auth-loading-spinner" /></div>
        ) : trips.length === 0 ? (
          ...
          <>
            <motion.div className="stats-cards" {...fadeUp(0.3)}>
              <div className="stats-card">
                <span className="stats-card_value">{String(trips.length).padStart(2, '0')}</span>
              ...
              <div className="stats-card">
                <span className="stats-card_value">{Object.keys(stats.regionList).length > 0 ? stats.regionList.length : '—'}</span>
                ...
                <div className="stats-region-list">
                  {stats.regionList.map(([region, count], i) => (
                    <div key={region} className="stats-region-item">
                      <span className="stats-region_rank">{String(i + 1).padStart(2, '0')}</span>
                      <span className="stats-region_name">{region}</span>
                      <span className="stats-region_count">{count} trip{count !== 1 ? 's' : ''}</span>
                    </div>
                  ))}
                </div>
              </motion.section>

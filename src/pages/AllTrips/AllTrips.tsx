...
export default function AllTrips() {
  ...
  const handleTag    = (tag: string)  => { setActiveTag(prev => prev === tag ? null : tag); setPage(1); };
      ...
      <div className="all-trips_inner">
        ...
        >
          <div className="all-trips_search-wrap">
            <svg className="all-trips_search-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
            ...
            />
            {query && (
              <button className="all-trips_search-clear" onClick={() => handleSearch('')} aria-label="Clear search">✕</button>
            )}
          </div>
            ...
            <div className="all-trips_tags">
              ...
              {(query || activeTag) && (
                <button
                  ...
                  onClick={() => { setQuery(''); setActiveTag(null); setPage(1); }}
                >
                  ✕ Clear filters
                </button>
              )}
        ...
        {loading ? (
          ...
          <div className="all-trips_empty">
            {trips.length === 0
              ? <p>No trips yet. Use <strong>+ Add Memory</strong> to start your journal.</p>
              : <p>No results for <strong>"{query || activeTag}"</strong>. Try a different search.</p>
            }
          </div>
        ) : (
          <div className="all-trips_grid">
            {visible.map((trip, i) => (
              ...
              >
                <Link to={`/trip/${trip.id}`} className="trip-grid-card_link">
                  ...
                  >
                    <img src={trip.cover} alt={trip.place} className="trip-grid-card_img" loading="lazy" />
                    <div className="trip-grid-card_overlay" />
                    <span className="trip-grid-card_index">{String(start + i + 1).padStart(2, '0')}</span>
                  </div>

                  <div className="trip-grid-card_body">
                    <div className="trip-grid-card_meta">
                      <span className="trip-grid-card_accent-dot" style={{ background: trip.accent }} />
                      <span className="trip-grid-card_region">{trip.region}</span>
                    </div>
                    ...
                    {(trip.tags ?? []).length > 0 && (
                      <div className="trip-grid-card_tags">
                        {trip.tags.map(t => <span key={t} className="trip-grid-card_tag">{t}</span>)}
                      </div>
                    )}
                    <span className="trip-grid-card_cta">
                      View Gallery<span className="trip-grid-card_arrow">→</span>
                    </span>
                  </div>
                </Link>
              </motion.div>
        ...
        {!loading && totalPages > 1 && (
          <div className="all-trips_pagination">
            <button className="all-trips_page-btn all-trips_page-btn-nav" onClick={() => goTo(page - 1)} disabled={page === 1} aria-label="Previous page">←</button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button key={p} className={`all-trips_page-btn${p === page ? ' all-trips_page-btn-active' : ''}`} onClick={() => goTo(p)} aria-label={`Page ${p}`} aria-current={p === page ? 'page' : undefined}>{p}</button>
            ))}
            <button className="all-trips_page-btn all-trips_page-btn-nav" onClick={() => goTo(page + 1)} disabled={page === totalPages} aria-label="Next page">→</button>
          </div>
        )}
      </div>
    </main>

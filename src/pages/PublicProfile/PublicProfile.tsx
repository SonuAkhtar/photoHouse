  ...
  if (notFound || !data) {
    return (
      <div className="pub-not-found">
        ...
        <Link to="/" className="pub-not-found_link">
          ← Back to home
        </Link>
      </div>
    );
    ...
    <main className="pub">
      <div className="pub_inner">
          ...
          <motion.div className="pub_gear" {...fadeUp(0.28)}>
            ...
            <div className="pub_gear-list">
              {profile.gear.map((item) => (
                <span key={item} className="pub_gear-tag">
                  {item}
                </span>
              ))}
            </div>
        ...
        <motion.div className="pub_trips-head" {...fadeUp(0.4)}>
          <p className="pub_trips-label">
            {trips.length} {trips.length === 1 ? "Journey" : "Journeys"}
          </p>
        </motion.div>

        {trips.length === 0 ? (
          <motion.p className="pub_empty" {...fadeUp(0.45)}>
            No trips shared yet.
          </motion.p>
        ) : (
                ...
                <div className="pub_card-body">
                  ...
                  {(trip.tags ?? []).length > 0 && (
                    <div className="pub_card-tags">
                      {trip.tags.map((t) => (
                        <span key={t} className="pub_card-tag">
                          {t}
                        </span>
                      ))}
                    </div>

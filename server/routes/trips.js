...
router.post('/', upload.array('photos', MAX_PHOTOS), async (req, res) => {
  try {
    ...
    if (!place || !region || !dates || !summary) {
      ...
      return res.status(400).json({ message: 'place, region, dates, and summary are required' });
    }
    ...
    captions = captions.map(c => typeof c === 'string' ? c.trim().slice(0, MAX_CAPTION_LEN) : '');
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (place   !== undefined) trip.place   = String(place).trim().slice(0, FIELD_LIMITS.place);
    if (region  !== undefined) trip.region  = String(region).trim().slice(0, FIELD_LIMITS.region);
    if (dates   !== undefined) trip.dates   = String(dates).trim().slice(0, FIELD_LIMITS.dates);
    if (summary !== undefined) trip.summary = String(summary).trim().slice(0, FIELD_LIMITS.summary);
    if (accent  !== undefined && /^#[0-9A-Fa-f]{6}$/.test(accent)) trip.accent = accent;
